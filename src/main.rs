        ...
        match event {
            ...
            WindowEvent::RedrawRequested => {
                ...
                match &self.view {
                    ...
                    View::Home { sel } => {
                        blit_menu(frame, |f| {
                            menu::render_home(f, *sel, self.nes.is_some(), self.error.as_deref())
                        });
                    }
                    View::Settings {
                        sel,
...
/// Draw a menu into `frame`. The menu art is laid out for the full 240-line
/// screen; when the window is shorter (NTSC overscan crop), render into a
/// full-height scratch buffer and blit the vertically centered slice so the
/// menu fills the window without bars or distortion.
fn blit_menu(frame: &mut [u8], draw: impl FnOnce(&mut [u8])) {
    ...
    draw(&mut full);
    let skip = (HEIGHT - h) / 2;
    frame.copy_from_slice(&full[skip * WIDTH * 4..(skip + h) * WIDTH * 4]);
}
