...
fn test_decimal_flag() {
    ...
    assert_eq!(
        nes.cpu.a, 0x0A,
        "Decimal ADC should still behave as binary (0x0A, not 0x10)"
    );
...
fn test_b_flag() {
    ...
    assert_eq!(
        nes.cpu.a & U,
        U,
        "PHP should set U flag (bit 5) in pushed status"
    );
    ...
    assert_eq!(
        nes.cpu.a & U,
        U,
        "BRK should set U flag (bit 5) in pushed status"
    );
}
...
fn test_dummy_read_cycles_lda_page_cross() {
    ...
    assert_eq!(
        nes.cpu.a & 0x80,
        0,
        "Dummy read should have cleared VBlank before final read"
    );
}
...
fn test_dummy_read_cycles_lda_no_page_cross() {
    let mut nes = make_nes(&[
        0xA2, 0x08, // LDX #$08
        0xBD, 0xF2,
        0x20, // LDA $20F2, X  (effective address $20FA, mirrors $2002, no page cross)
        0x00, // BRK
    ]);
    ...
    assert_eq!(
        nes.cpu.a & 0x80,
        0x80,
        "No page cross -> no dummy read -> final read should see VBlank"
    );
}
...
fn test_dummy_read_cycles_sta_always() {
    let mut nes = make_nes(&[
        ...
        0xA2, 0x10, // LDX #$10
        0x9D, 0xF2,
        0x20, // STA $20F2, X  (effective address $2102, mirrors $2002, uncorrected $2002, page cross)
        0x00, // BRK
    ]);
    ...
    assert_eq!(
        status & 0x80,
        0,
        "STA dummy read should have cleared VBlank"
    );
}
...
fn test_open_bus_lda_absolute() {
    ...
    assert_eq!(
        nes.cpu.a, 0x55,
        "LDA from unmapped $55AA should return high byte of operand (0x55)"
    );
}
...
fn test_open_bus_controller_bits() {
    ...
    assert_eq!(
        val & 0xE0,
        0xE0,
        "Upper 3 bits of controller read should be open bus"
    );
}
...
fn test_open_bus_4015_read_bit5() {
    ...
    assert_eq!(
        val & 0x20,
        0x20,
        "Bit 5 of $4015 read should be internal bus bit 5"
    );
}
...
fn test_abs_indexed_wraparound() {
    ...
    assert_eq!(
        nes.cpu.a, 0xE7,
        "Absolute indexed past $FFFF should wrap to Zero Page"
    );
}
...
fn test_zp_indexed_wraparound() {
    ...
    assert_eq!(
        nes.cpu.a, 0x88,
        "ZP indexed past $FF should wrap within ZP ($0000-$00FF)"
    );
}
...
fn test_jmp_indirect_page_wrap_bug() {
    ...
    assert_eq!(
        nes.cpu.pc, 0x6C34,
        "JMP indirect at page boundary should wrap to page start for high byte"
    );
}
...
fn test_unofficial_sax() {
    ...
    nes.cpu.step(); // SAX
    assert_eq!(
        nes.cpu.bus.read(0x0020),
        0x00,
        "SAX should write A & X (0x0F & 0xF0 = 0x00)"
    );
}
